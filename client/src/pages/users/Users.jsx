...
export function Users() {
  ...
  // New user form state. badgeNumber is never set by the form — it's
  // always assigned by the server from the role's prefix (AD-/EO-/SV-/
  // CS-/CI-), shown here only as a read-only preview.
  ...
  const [nextBadgePreview, setNextBadgePreview] = useState("");
  ...
  // Refresh the "next badge number" preview whenever the create modal is
  // open and the selected role changes.
  useEffect(() => {
    if (!isCreateOpen) return;
    let cancelled = false;
    setIsPreviewLoading(true);
    userApi
      .previewNextBadge(formData.role)
      .then((res) => {
        if (!cancelled && res.success) {
          setNextBadgePreview(res.data.badgeNumber);
        }
      })
      .catch(() => {
        if (!cancelled) setNextBadgePreview("");
      })
      .finally(() => {
        ...
        <form onSubmit={handleCreateUser} className="space-y-3 text-xs">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <div
                className="w-full font-mono uppercase p-2 rounded-lg border border-dashed border-slate-300 bg-slate-50 text-slate-500"
                title="Badge numbers are assigned automatically based on role and cannot be edited."
              >
                {isPreviewLoading ? "…" : nextBadgePreview || "—"}
              </div>
              <p className="text-[10px] text-slate-400 mt-1">
                Auto-assigned on save (AD-/EO-/SV-/CS-/CI- + sequence)
              </p>
            </div>
          </div>
