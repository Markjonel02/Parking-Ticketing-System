...
import { Users as UsersIcon, PlusCircle, ShieldCheck, UserCheck, UserX } from 'lucide-react';
...
export function Users() {
  ...
  async function handleToggleStatus(targetUser) {
    try {
      ...
      showToast({ title: 'Update Failed', description: err.message, status: 'error' });
    }
  }
  ...
  async function handleCreateUser(e) {
    ...
    if (!formData.name || !formData.email) {
      showToast({ title: 'Validation Error', description: 'Name and email are required.', status: 'error' });
      return;
    }
    ...
    try {
      ...
      if (res.success) {
        showToast({ title: 'Staff Provisioned', description: `${formData.name} added to roster.`, status: 'success' });
      ...
      showToast({ title: 'Provisioning Failed', description: err.message, status: 'error' });
    }
  }
  ...
  const columns = [
    {
      ...
      render: (u) => (
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <span className="font-semibold text-xs text-slate-900 block">{u.name}</span>
    ...
    },
    {
      ...
      key: 'role',
      render: (u) => (
        <span className={`px-2 py-0.5 rounded text-[11px] font-bold ${roleColors[u.role] || roleColors.OFFICER}`}>
          {u.role}
        </span>
      )
    },
    ...
    },
    {
      ...
      key: 'department',
      render: (u) => <span className="text-xs text-slate-600">{u.department || 'Enforcement'}</span>
    },
    {
      header: 'Status',
      key: 'status',
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Role assignments, badge credentialing, and officer department administration
          </p>
        </div>

        <Button size="sm" colorScheme="brand" onClick={() => setIsCreateOpen(true)} leftIcon={<PlusCircle className="w-4 h-4" />}>
          Provision Officer
        </Button>
      </div>
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="outline" colorScheme="gray" size="sm" onClick={() => setIsCreateOpen(false)}>
              Cancel
            </Button>
      ...
      >
        <form onSubmit={handleCreateUser} className="space-y-3 text-xs">
          ...
          <div>
            <label className="block font-semibold text-slate-700 mb-1">Municipal Email *</label>
            <input
              type="email"
              required
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block font-semibold text-slate-700 mb-1">Badge / Shield ID</label>
              <input
                type="text"
